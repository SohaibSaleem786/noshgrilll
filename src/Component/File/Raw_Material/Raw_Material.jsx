...
function Raw_Material() {
  ...
  const apiLinkss = `${apiLinks}/RawMaterialList.php`;

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`${apiLinks}/RawMaterialList.php`);
        if (!response.ok) {
  ...
  const fetchnewdata = async () => {
    try {
      const response = await fetch(`${apiLinks}/RawCode.php`);
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      formData.append("rawdsc", Description.current.value);
      formData.append("rawsts", value.cstss);
      formData.append("rawuom", value.unit);
      formData.append("code", Code.current.value);
      // formData.append("userid", 33);
      axios
        .post(`${apiLinks}/AddRawMaterial.php`, formData, {
  ...
  const [textdata, settextdata] = useState("Raw Material");
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "AccountCodeform") {
      ...
      const selectedItem = dataa.rows.find(
        (item) => item.raw_cod === formattedValue
      );

      console.log("Selected item:", selectedItem);

      if (selectedItem) {
        setAccountCodeform(selectedItem.raw_cod);
        setDescriptionform(selectedItem.raw_dsc);
        setSelectedStatus(selectedItem.raw_sts);
        setSelectedUnit(selectedItem.raw_uom);
      } else {
        setDescriptionform("");
  ...
  const filteredRows = data.rows.filter(
    (row) =>
      ...
      (row.raw_sts &&
        row.raw_sts.toLowerCase().includes(searchText.toLowerCase()))
  );
  ...
  const handleRowClick = (rowData, rowIndex) => {
    ...
    setModalOpen(false);
    setHighlightedRowIndex(0);
    setSelectedUnit(rowData.raw_uom);

    setSelectedStatus(rowData.raw_sts);
    setDescriptionform(rowData.raw_dsc);
    setAccountCodeform(rowData.raw_cod);
    resetData();
  };
            ...
            <div className="col-md-12 form-accountcode-container">
              ...
              <Form>
                <div className="row scroll-account">
                  <div className="col-12">
                      ...
                      <div className="col-sm-10 input-account">
                        ...
                        >
                          <Form.Control
                            ...
                            onKeyDown={(e) => handleEnterKeyPress(unit, e)}
                            ref={Description}
                            onFocus={(e) => e.target.select()}
                          />
                        </Form.Group>
                    ...
                    <div className="row">
                      <div className="col-sm-2 account-field">Unit:</div>
                      <div className="col-sm-4 input-account">
                        <Form.Group
                          controlId="status"
                          style={{ display: "flex", alignItems: "center" }}
              ...
              >
                ...
                >
                  ...
                  >
                    <strong>Select Raw Material</strong>
                  </div>
                  <div className="text-end col-4">
                    <Link
                      onClick={() => setModalOpen(false)}
