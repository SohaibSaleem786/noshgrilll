...
function Cash_Payment_Voucher() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`${apiLinks}/CPVNumber.php`);
        if (!response.ok) {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${apiLinks}/CashPaymentVoucher.php`,
      ...
      if (response.data.error === 200) {
        ...
        // call the next code functon and pass the nextItemId and empty all field
        setTableData([
          {
            name: "",
            Description: "",
